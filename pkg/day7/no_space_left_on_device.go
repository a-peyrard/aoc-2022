...
much louder sounds in the distance; how big do the animals get out here, anyway? The device the Elves gave you has
problems with more than just its communication system. You try to run a system update:

$ system-update --please --pretty-please-with-sugar-on-top
Error: No space left on device

Perhaps you can delete some files to make space for the update? You browse around the filesystem to assess the situation
and save the resulting terminal output (your puzzle input). For example:
...
listing the contents of the directory you're currently in. Within the terminal output, lines that begin with $ are
commands you executed, very much like some modern computers:
    ...
    the argument:
        ...
        that directory the current directory. cd / switches the current directory to the outermost directory, /.
...
directories also contain files of various sizes. Since the disk is full, your first step should probably be to find
directories that are good candidates for deletion. To do this, you need to determine the total size of each directory.
The total size of a directory is the sum of the sizes of the files it contains, directly or indirectly. (Directories
themselves do not count as having any intrinsic size.) The total sizes of the directories above can be found as follows:

    The total size of directory e is 584 because it contains a single file i of size 584 and no other directories. The
    directory a has total size 94853 because it contains files f (size 29116), g (size 2557), and h.lst (size 62596),
    plus file i indirectly (a contains e which contains i). Directory d has total size 24933642. As the outermost
    directory, / contains every file. Its total size is 48381165, the sum of the size of every file.
...
func (parent *file) add(f *file) *file {
...
func parse(input string) *file {
	currentPath := collection.Stack[*file]{}
	rootFile := newDirectory("/")
	currentPath.Push(rootFile)

	var currentFile *file
	sc := bufio.NewScanner(strings.NewReader(input))
	sc.Scan() // ignore the first '$ cd /'
	fileName, size := "", 0
	for sc.Scan() {
		line := sc.Text()
		if line[0] != '$' {
			if strings.HasPrefix(line, "dir ") {
				currentFile = newDirectory(line[4:])
			} else {
				size, fileName = parseFile(line)
				currentFile = newFile(fileName, size)
			}
			currentPath.Peek().add(currentFile)
		} else {
			// analyze cd commands, we can safely ignore ls
			if line == "$ cd .." {
				currentPath.Pop()
			} else if strings.HasPrefix(line, "$ cd ") {
				// $ cd [some directory]
				fileName = line[5:]
				currentPath.Push(currentPath.Peek().files[fileName])
			}
		}
	}
	return rootFile
...
func parseFile(raw string) (int, string) {
	matches := fileRegex.FindStringSubmatch(raw)
	size, _ := strconv.Atoi(matches[1])

	return size, matches[2]
}

func findDirectoriesBySize(file *file, predicate func(int) bool, results *[]int) int {
	if !file.isDirectory {
		return file.size
	}
	size := 0
	for _, file := range file.files {
		size += findDirectoriesBySize(file, predicate, results)
	}
	if predicate(size) {
...
func DoSolution1(raw string) int {
	root := parse(raw)
	result := make([]int, 0)
	findDirectoriesBySize(root, func(size int) bool { return size < 100_000 }, &result)

	return collection.Sum(&result)
}
