...
range of section IDs. However, as some of the Elves compare their section assignments with each other, they've noticed
that many of the assignments overlap. To try to quickly find overlaps and reduce duplicated effort, the Elves pair up
and make a big list of the section assignments for each pair (your puzzle input). For example, consider the following
...
For the first few pairs, this list means:
    ...
    was assigned sections 6-8 (sections 6, 7, 8). The Elves in the second pair were each assigned two sections. The
    Elves in the third pair were each assigned three sections: one got sections 5, 6, and 7, while the other also got 7,
    plus 8 and 9.
...
func (s *Section) fullyOverlap(other *Section) bool {
...
func parse(input string) []Pair {
	...
	for sc.Scan() {
		rawSections := strings.Split(sc.Text(), ",")
		res = append(
			res,
			Pair{
				parseSection(rawSections[0]),
				parseSection(rawSections[1]),
			},
		)
	}
	return res
...
func parseSection(rawSection string) Section {
	rawNumbers := strings.Split(rawSection, "-")
	start, _ := strconv.Atoi(rawNumbers[0])
	end, _ := strconv.Atoi(rawNumbers[1])
